...
namespace ProjectNghiPhep
{
    ...
    public static class MVCGridConfig 
    {
        public static void RegisterGrids()
        {
            ...
            MVCGridDefinitionTable.Add("ProcessDocumentGridEm", new MVCGridBuilder<Document>()
                .WithAuthorizationType(AuthorizationType.AllowAnonymous)
                .WithSorting(sorting: true, defaultSortColumn: "Id", defaultSortDirection: SortDirection.Dsc)
                //.WithPaging(true, 5)
                .WithAdditionalQueryOptionNames("search")
                .AddColumns(cols =>
                {
                    ...
                    cols.Add("RemoveBtn").WithSorting(false)
                        .WithHeaderText("")
                        .WithHtmlEncoding(false)
                        .WithValueExpression((p, c) => c.UrlHelper.Action("RemoveDocument", "DonNghiPhep", new { id = p.C_id }))
                        .WithValueTemplate("<a href='{Value}' class='btn btn-danger btn-verify' role='button' data-form-method='post' onclick=\"return confirm(\'Bạn có muốn xóa đơn?\')\">Xóa</a>");
                    cols.Add("UpdateBtn").WithSorting(false)
                        .WithHeaderText("")
                        .WithHtmlEncoding(false)
                        .WithValueExpression((p, c) => c.UrlHelper.Action("UpdateDocument", "DonNghiPhep", new { id = p.C_id }))
                        .WithValueTemplate("<a href='{Value}' class='btn btn-primary'>Chi tiết</a>");
                    cols.Add("publishBtn").WithSorting(false)
                        .WithHeaderText("")
                        .WithHtmlEncoding(false)
                        .WithValueExpression((p, c) => c.UrlHelper.Action("PublishDocument", "DonNghiPhep", new { id = p.C_id }))
                        .WithValueTemplate("<a href='{Value}' class='btn btn-primary btn-verify' role='button' data-form-method='post' onclick=\"return confirm(\'Bạn có muốn nộp đơn?\')\">Nộp đơn</a>");
                })
                .WithRetrieveDataMethod((context) =>
                {
                    var result = new QueryResult<Document>();
                    ...
                    var query = (from doc in db.Documents
                                 join user in db.Users
                                 on doc.createdById equals user.C_id
                                 where doc.status == -1 && doc.createdById == u.C_id
                                 select new
                                 {
                                     C_id = doc.C_id,
                                     createdBy = user,
